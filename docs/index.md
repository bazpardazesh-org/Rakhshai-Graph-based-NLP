...
- **Persian Graph-LM path**: `PersianTokenizer`, `build_graph_lm_graph`, `GraphCausalLM` and `LMTrainer` provide a causal LM baseline with optional graph encoding and graph-token fusion. The stable v1 architecture is documented in [Rakhshai Graph-LM v1 Architecture](graph_lm_v1.md), and the Phase 3 multi-relation graph is documented in [Rakhshai Multi-Relation Persian Graph](multi_relation_persian_graph.md).
